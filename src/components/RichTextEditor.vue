...
function applyBlockType(value: string): void {
  if (value === "paragraph") {
    focusCommand((chain) => chain.setParagraph().run());
    return;
  }
  const level = Number(value.slice(1));
  if (level >= 1 && level <= 3) {
    focusCommand((chain) => chain.toggleHeading({ level: level as 1 | 2 | 3 }).run());
  }
}
  ...
  <section class="rich-text-editor">
    ...
    <div class="rich-format-toolbar" aria-label="正文格式工具栏" @click.stop>
      <select
        ...
        @change="applyBlockType(($event.target as HTMLSelectElement).value)"
      >
